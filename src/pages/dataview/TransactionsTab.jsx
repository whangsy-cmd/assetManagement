...
import { getTransactionsByAccount, deleteDocumentsByIds } from '../../utils/firestore'
...
export default function TransactionsTab() {
  ...
  const [modal, setModal] = useState(null) // { type: 'date'|'account', date?, accountId?, count }
  ...
  const codeOptions = [...new Map(data.filter(d => d.code).map(d => [d.code, d.name || d.code])).entries()]
    .sort((a, b) => (a[1] || '').localeCompare(b[1] || ''))

  const sorted = [...data].sort((a, b) => b.date.localeCompare(a.date))
    .filter(d => selectedDate === '전체' || d.date === selectedDate)
    .filter(d => selectedType === '전체' || d.type === selectedType)
    .filter(d => selectedCode === '전체' || d.code === selectedCode)

  const totalAmount = sorted.reduce((s, r) => s + (r.amount || 0), 0)
  const totalFee = sorted.reduce((s, r) => s + (r.fee || 0), 0)
  const totalTax = sorted.reduce((s, r) => s + (r.tax || 0), 0)

  // 날짜/거래종류/종목 등 현재 걸린 필터를 전부 반영한 sorted(화면에 보이는 행) 기준으로 삭제 — 필터 일부만 반영해 다른 계좌/조건 데이터까지 지워지는 것 방지
  const handleDelete = async () => {
    setDeleting(true)
    await deleteDocumentsByIds(user.uid, 'transactions', sorted.map(r => r.docId))
    setModal(null)
      ...
      <div className="toolbar">
        <div className="date-row">
          ...
          {selectedAccount && (
            <>
              ...
              <span className="tool-label">거래종류</span>
              ...
              <select value={selectedCode} onChange={e => setSelectedCode(e.target.value)} className="select input-sm" style={{ maxWidth: 260 }}>
                <option value="전체">전체</option>
                {codeOptions.map(([code, name]) => <option key={code} value={code}>{name}</option>)}
              </select>
            </>
          )}
        </div>
        {selectedAccount && (
          <div className="tool-right">
            ...
            {selectedDate !== '전체' && (
              <button className="btn btn-outline-orange btn-sm" onClick={() => setModal({ type: 'date', date: selectedDate, count: sorted.length })}>
                {selectedDate} 삭제
              </button>
            )}
            <button className="btn btn-outline-orange btn-sm" onClick={() => setModal({ type: 'account', accountId: selectedAccount, count: sorted.length })}>
              {selectedAccount} 삭제
            </button>
          </div>
        )}
      ...
      {!selectedAccount ? (
        ...
        <div className="summary-bar" style={{ marginBottom: 12, padding: '8px 16px', justifyContent: 'flex-end' }}>
          <div className="summary-item" style={{ flex: 'none', flexDirection: 'row', gap: 6, alignItems: 'baseline' }}>
            <span className="summary-label">거래금액</span>
            <span className="summary-item-val" style={{ fontSize: 14 }}>{fmt(totalAmount)}</span>
          </div>
          <div className="summary-item" style={{ flex: 'none', flexDirection: 'row', gap: 6, alignItems: 'baseline' }}>
            <span className="summary-label">수수료</span>
            <span className="summary-item-val" style={{ fontSize: 14 }}>{fmt(totalFee)}</span>
          </div>
          <div className="summary-item" style={{ flex: 'none', flexDirection: 'row', gap: 6, alignItems: 'baseline' }}>
            <span className="summary-label">세금</span>
            <span className="summary-item-val" style={{ fontSize: 14 }}>{fmt(totalTax)}</span>
          </div>
          <div className="summary-item" style={{ flex: 'none', flexDirection: 'row', gap: 6, alignItems: 'baseline' }}>
            <span className="summary-label">수수료+세금</span>
            <span className="summary-item-val" style={{ fontSize: 14 }}>{fmt(totalFee + totalTax)}</span>
          </div>
        </div>
